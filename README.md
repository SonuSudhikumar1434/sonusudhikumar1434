# 👋 Welcome to my GitHub!

## Turning data into insights through analytics, engineering, and visualization

Hi, I'm **SONU S** — an Electrical & Electronics Engineer and Data Analytics enthusiast passionate about building data-driven solutions and interactive dashboards.

I specialize in SQL-based data transformation, healthcare analytics, dashboard visualization, and data warehousing using modern analytics workflows.

---

# 🛠️ Core Skills

### 📊 Data Analytics & Engineering
- SQL
- Python
- Advanced Excel
- DuckDB
- Data Modeling
- ETL / ELT Workflow
- Medallion Architecture

### 📈 Visualization & BI
- Looker Studio
- Google Sheets
- KPI Dashboard Development
- Data Visualization

### ⚙️ Tools & Technologies
- Git & GitHub
- VS Code
- AWS Basics
- AutoCAD
- WordPress

---

# 🧠 What I Do

- Build end-to-end analytics workflows using SQL and DuckDB
- Create Bronze-Silver-Gold Medallion Architecture pipelines
- Develop interactive KPI dashboards in Looker Studio
- Transform raw healthcare data into business-ready insights
- Perform data cleaning, validation, and transformation
- Work on renewable energy and solar engineering projects

---

# 📂 Featured Projects
project links:https://github.com/SonuSudhikumar1434?tab=repositories

## Dashboard Preview
screenshots/Hospital Performance Dashboard.png

![Dashboard](screenshots/dashboard.png)

## 🏥 Hospital Data Analytics Project
End-to-end healthcare analytics pipeline using:
- DuckDB
- SQL
- Google Sheets
- Looker Studio

### Features
✔ Bronze-Silver-Gold Architecture  
✔ Patient Analytics Dashboard  
✔ Revenue KPI Analysis  
✔ Alive vs Dead Analysis  
✔ Gender Composition Dashboard  
✔ Healthcare Reporting Marts  

---

## ☀️ Solar Engineering Projects

- 40KW Solar Power Plant Project
- Solar Site Coordination & Monitoring
- Electrical Distribution Systems
- Project Scheduling & Safety Monitoring

---

# 🎓 Certifications


- Data Analyst Course
  - Advanced Excel
  - SQL
  - Python
  - Looker Studio
- NEBOSH International General Certificate (IGC) 

---

# 🌱 Currently Learning

- Advanced SQL Optimization
- Dashboard Automation
- Business Intelligence Workflows

---

# 📬 Get In Touch

Email: sonusudhikumarjobs@gmail.com


linkedin: https://www.linkedin.com/in/sonu-s-6b3b9421a/
---

# ⚡ Beyond Tech

- IEEE Student Branch Chair
- NSS Coordinator
- Solar Consultant
- YouTuber
- Short Film Maker

---

# 🚀 Thanks for Visiting!

Always open to:
- Collaboration
- Learning opportunities
- Data analytics projects
- Engineering innovations
